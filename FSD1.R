...
fo_stodom(time_series_list$D1,time_series_list$H1)
...
fo_stodom(time_series_list[combinations_df$Series1],time_series_list[combinations_df$Series2])
...
for(i in seq_along(combinations_df$Series1)){
  fo_stodom(time_series_list[combinations_df$Series1][[i]],time_series_list[combinations_df$Series2][[i]]))
}



# Assuming fo_stodom is a function that returns a numeric value, like the SSD
...
# Loop through each combination and calculate the fo_stodom value
for(i in seq_along(combinations_df$Series1)){
  ...
  ts1 <- time_series_list[[series1_name]]
  ts2 <- time_series_list[[series2_name]]
  
  # Calculate the result using the fo_stodom function (assuming it returns a numeric value)
  result <- fo_stodom(ts1, ts2)
  
  # Store the results in the data frame
  results_df <- rbind(results_df, data.frame(Series1 = series1_name, Series2 = series2_name, Result = result))
}
